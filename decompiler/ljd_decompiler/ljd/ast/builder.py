...
def _build_function_blocks(state, instructions):
	...
	state._instructions = instructions
	...
	for block in state.blocks:
		...
		while addr <= block._last_body_addr:
			...
			if statement is not None:
				...
				# KL-specific: TGETS/TGETV/TGETB implicitly copy
				# the table reference (slot B) into slot A+1.
				# Synthesize a MOV assignment BEFORE the TGET so
				...
				if opcode >= ins.TGETV.opcode \
					...
					synth.destinations.contents.append(
						_build_slot(state, addr,
							instruction.A + 1))
					synth.expressions.contents.append(
						_build_slot(state, addr,
							instruction.B))
					setattr(synth, "_addr", addr)
					setattr(synth, "_line", line)
					block.contents.append(synth)
...
def _build_warp(state, last_addr, instructions):
	if not instructions:
		# Empty instructions list — generate a simple unconditional warp
		warp = nodes.UnconditionalWarp()
		warp.type = nodes.UnconditionalWarp.T_FLOW
		warp.target = state.blocks[0]  # dummy, will be resolved later
		return warp, 0
...
def _build_statement(state, addr, instruction):
	...
	# ASSIGNMENT starting from TGETV and ending at TGETB
		...
		if not (opcode == ins.UCLO.opcode or (
				opcode >= ins.LOOP.opcode
						and opcode <= ins.JLOOP.opcode)):
			# Unmapped or misidentified KL opcode — treat as no-op
			# Known case: KL50 (IST=14) appears without JMP in some files
			pass
		# Noop
		return None
...
def _build_binary_expression(state, addr, instruction):
	...
	# NV opcodes: A = numconst[CD] OP slot[B] (left=const, right=var)
	_NV_OPCODES = {ins.ADDNV.opcode, ins.SUBNV.opcode, ins.MULNV.opcode,
				   ins.DIVNV.opcode, ins.MODNV.opcode}

	if instruction.B_type == ins.T_VAR and instruction.CD_type == ins.T_VAR:
		# VV: both operands are slots
		operator.left = _build_slot(state, addr, instruction.B)
		operator.right = _build_slot(state, addr, instruction.CD)
	elif instruction.B_type == ins.T_VAR and opcode in _NV_OPCODES:
		# NV: A = numconst[CD] OP slot[B]
...
def _build_table_copy(state, slot):
	...
	for key, value in table.dictionary:
		# Skip nil-valued template entries.  LuaJIT pre-allocates hash
		# slots with nil values; the real values are set by subsequent
		# TSETS/TSETB instructions that get folded via insert_table_record.
		# Emitting them here causes duplicate keys (key = nil, key = REAL).
		if value is None:
			continue
...
def _build_numeric_constant(state, index):
	if index >= len(state.constants.numeric_constants):
		# Safety fallback for any residual opcode mapping edge cases
		node = nodes.Constant()
