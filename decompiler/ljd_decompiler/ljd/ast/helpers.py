...
def _keys_equal(a, b):
	"""Compare two table record keys for equality."""
	if type(a) != type(b):
		return False
...
def insert_table_record(constructor, key, value):
	...
	# Check for duplicate key — replace existing record instead of appending
	for i, existing in enumerate(records):
		if isinstance(existing, nodes.TableRecord) \
				and _keys_equal(existing.key, key):
