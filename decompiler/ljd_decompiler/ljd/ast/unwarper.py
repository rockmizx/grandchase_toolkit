...
def _run_step(step, node, **kargs):
	for statements in _gather_statements_lists(node):
		try:
			...
			pass  # Leave this scope's blocks as-is
...
def _glue_flows(node):
	for statements in _gather_statements_lists(node):
		...
		# Check all blocks are flow-through before gluing
		can_glue = True
		for i, block in enumerate(blocks[:-1]):
			if not _is_flow(block.warp) or block.warp.target != blocks[i + 1]:
				can_glue = False
				break
...
def _unwarp_ifs(blocks, top_end=None, topmost_end=None):
	...
	while start_index < len(blocks) - 1:
		...
		if body is None:
			# GOTO or unrecognized pattern — skip this block
			start_index += 1
			continue
		...
		try:
			...
			# Skip this if-block, leave raw blocks in place
			start_index += 1
			continue
...
def _search_expression_end(expression, falses):
	...
	if false is None:
		raise ValueError("_search_expression_end: no false target found")
