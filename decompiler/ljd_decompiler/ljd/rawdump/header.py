...
_MAGIC = b'\x1bLJ'
_MAGIC_KL = b'\x1bKL'  # KOG/GrandChase custom LuaJIT

_MAX_VERSION = 0x84  # Extended for KOG custom bytecode (was 0x80)
...
def _check_magic(state):
	magic = state.stream.read_bytes(3)
	if magic != _MAGIC and magic != _MAGIC_KL:
...
def _read_flags(parser, header):
	...
	if bits != 0:
		pass  # Silently ignore unknown flags for KOG custom bytecode
