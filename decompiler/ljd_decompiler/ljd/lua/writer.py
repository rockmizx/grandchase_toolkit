...
def _nodes_equal(a, b):
	"""Semantic equality for AST nodes (used to detect method calls)."""
	if a is b:
		return True
	if type(a) is not type(b):
		return False
	if isinstance(a, nodes.Identifier):
		return a.slot == b.slot and a.type == b.type
	if isinstance(a, nodes.TableElement):
		return _nodes_equal(a.table, b.table) and _nodes_equal(a.key, b.key)
	if isinstance(a, nodes.Constant):
		return a.type == b.type and a.value == b.value
	if isinstance(a, nodes.Primitive):
		return a.type == b.type
	if isinstance(a, nodes.MULTRES):
		return True
	return False
...
class Visitor(traverse.Visitor):
	...
	def visit_function_call(self, node):
		...
		if len(args) > 0 and isinstance(node.function, nodes.TableElement):
			...
			if self._is_valid_name(node.function.key):
				is_method = _nodes_equal(table, first_arg)
		...
		if is_method:
			...
			# KL-specific: TGETS implicit self means the compiler
			# also generates an explicit self copy.  After removing
			# the implicit self (above), the explicit self may still
			# be the first remaining arg — remove it too for clean
			# colon-style output.
			if len(args) > 0 \
					and _nodes_equal(node.function.table, args[0]):
				args.pop(0)
	...
	def visit_unconditional_warp(self, node):
		# Suppress all warp annotations for cleaner output
		pass

	def visit_conditional_warp(self, node):
		# Emit minimal if-then for unresolved conditional warps
	...
	def visit_constant(self, node):
		if node.type == nodes.Constant.T_INTEGER:
			self._write(node.value)
			return

		if node.type == nodes.Constant.T_FLOAT:
			# Format float: strip trailing zeros but keep at least one decimal
			val = node.value
			if isinstance(val, (int, float)):
				...
				if val == float('-inf'):
					self._write("-1/0")
					return
				# Integer-valued floats → show as integer
				if val == int(val) and abs(val) < 2**53:
					self._write(str(int(val)))
					return
				self._write(str(val))
				return
			self._write(node.value)
	...
	for i, cmd in enumerate(queue):
		...
		if cmd[0] == CMD_START_STATEMENT:
			...
			if next_cmd[0] not in (CMD_END_BLOCK, CMD_START_BLOCK):
				...
				# Only add blank line before/after if/while/for/repeat/function
				if cmd[1] >= STATEMENT_IF or next_cmd[1] >= STATEMENT_IF:
					fd.write("\n")
